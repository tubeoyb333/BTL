...
class MathExpression:
    def __init__(self, expression_str):
        # Lưu biểu thức toán học dưới dạng chuỗi và biến 'x' là biến độc lập
        self.expression_str = expression_str
        self.x = sp.symbols('x')
        try:
            # Chuyển đổi chuỗi biểu thức thành đối tượng biểu thức toán học
...
class MathAppGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.master, text="Nhập hàm số (sử dụng 'x'):",font=bigger_font).grid(row=0, column=0, pady=10, padx=10, sticky=tk.W)
        ...
        if file_path:
            try:
                ...
                if 'function' in data_frame.columns and not data_frame['function'].empty:
                    ...
                    self.show_error("Tệp tin Excel không chứa cột 'function' hoặc cột 'function' trống.")
            except pd.errors.EmptyDataError:
                self.show_error("Tệp tin Excel không chứa dữ liệu.")
            except Exception as e:
                self.show_error(f"Lỗi khi đọc tệp tin Excel. Định dạng file có thể không đúng hoặc có lỗi khác.")
    ...
    def validate_variable(self, expression_str):
        try:
            ...
            # Lấy tất cả các biến trong biểu thức
    ...
    def calculate_derivative(self):
        ...
        if not self.validate_variable(expression_str):
            return
        try:
            # Tạo đối tượng MathExpression để thực hiện các phép toán
    ...
    def find_extreme_points(self):
        ...
        x = math_expression.x
        expression = math_expression.expression
        # Tìm điểm critical của hàm số
        critical_points = sp.solveset(sp.diff(expression, x), x, domain=sp.S.Reals)
        self.extreme_points = []
        # Xác định loại điểm cực trị (cực đại/cực tiểu)
    ...
    def show_result(self, message):
            # Cập nhật nội dung nhãn hiển thị kq
    ...
    def save_result(self):
        ...
        if file_path:
            try:
                ...
                self.show_error(f"Lỗi khi lưu kết quả")
def main():
    root = tk.Tk()
    app = MathAppGUI(root)
    root.mainloop()
